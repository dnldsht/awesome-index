...
4. **Mount `SearchApp` on the list page itself**, with the `list` facet locked
   ...
   - **The curator's note and the section are not in the index.** `RepoCard`
     ...
     facet. Both are fixable from `r/[owner]/[repo].astro` — a per-list
     `note_<slug>` meta, a `section:golang/web-frameworks` filter — but that is
     N more tagged values per repository page, and it is what turns this from a
     small change into a medium one.
